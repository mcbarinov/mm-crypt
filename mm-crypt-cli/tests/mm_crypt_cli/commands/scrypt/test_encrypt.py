...
class TestKdfParams:
    ...
    def test_log_n_out_of_range(self, runner, err_text):
        ...
        assert "log_n must be in" in err_text(result)

    def test_r_out_of_range(self, runner, err_text):
        ...
        assert result.exit_code == 1
        assert "r must be in" in err_text(result)

    def test_p_out_of_range(self, runner, err_text):
        ...
        assert result.exit_code == 1
        assert "p must be in" in err_text(result)
