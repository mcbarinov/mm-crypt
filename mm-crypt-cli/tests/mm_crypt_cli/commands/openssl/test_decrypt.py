...
class TestDecryptErrors:
    ...
    def test_missing_salt_header(self, runner, err_text):
        ...
        assert result.exit_code == 1
        assert "missing OpenSSL salt header" in err_text(result)

    def test_binary_missing_salt_header(self, runner, err_text):
