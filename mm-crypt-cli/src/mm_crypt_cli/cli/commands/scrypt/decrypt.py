...
def decrypt(
    ...
    try:
        if binary:
            plaintext_bytes = scrypt.decrypt_bytes(data=read_bytes_input(input_file), password=resolved_password)
            write_bytes_output(plaintext_bytes, output_file)
        else:
            plaintext = scrypt.decrypt_base64(data=read_text_input(input_file), password=resolved_password)
