...
def register(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    parser = subparsers.add_parser(
        "decrypt",
        aliases=["d"],
        help="Decrypt AES-256-CBC ciphertext (OpenSSL-compatible).",
        description="Decrypt from --input (or stdin) using AES-256-CBC with PBKDF2; OpenSSL-compatible.",
    )
    parser.add_argument(
...
def _run(args: argparse.Namespace) -> None:
    """Decrypt from the chosen source using AES-256-CBC."""
    password = resolve_secret(
        value=args.password, file=args.password_file, env=args.password_env, flags=_PASSWORD_FLAGS, label="password"
    )
    try:
        if args.binary:
            plaintext_bytes = openssl_aes256cbc.decrypt_bytes(data=read_bytes_input(args.input_file), password=password)
            write_bytes_output(plaintext_bytes, args.output_file)
        else:
            plaintext = openssl_aes256cbc.decrypt_base64(data=read_text_input(args.input_file), password=password)
